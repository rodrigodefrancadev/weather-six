...
export default class OpenWeatherApi {
  ...
  public async getOneCallApi(latLon: LatLon): Promise<OneCallApiData> {
    const url = `https://api.openweathermap.org/data/3.0/onecall?units=metric&lat=${latLon.lat}&lon=${latLon.lon}&exclude=minutely,daily,alerts&appid=${this.apiKey}`;
    ...
    if (response.status === 200) {
      ...
      throw new Error(
        "Weather not found to this location: " + JSON.stringify(latLon)
      );
    }
  }
